...
class Individual(object):
    def __init__(self, num_points, height, width):
        self.place_genotype = np.random.choice(range(height), p=np.ones(height) /height, size=3 * num_points)
    ...
    def get_params(self):
        return self.place_genotype.reshape((-1, 3)), self.color_genotype.reshape((-1, 4))
...
def evaluate_individuals(individuals, ground_t,width,height, trasnparent):
    for ind in individuals:
        ...
        if trasnparent:
            im = draw_circles_shaded(veroni_points,veroni_colors,width,height)
        else:
            painting = draw_circle(veroni_points,veroni_colors,width,height)
...
def main(display=False, interval = 100, verbose_interval = 1, transparent = True):
    ...
    for i in range(itterations):
        ...
        if display and i % interval == 0:
            ...
            if not transparent:
                painting = draw_circle(veroni_points*(1/scale), veroni_colors, width, height)
            if transparent:
                painting = np.asarray(draw_circles_shaded(veroni_points * (1 / scale), veroni_colors, width, height))
