...
class Individual(object):
    def __init__(self, num_points, height, width,poly_count):
        self.place_genotype = np.random.choice(range(height), p=np.ones(height) /height, size=2*poly_count * num_points)
        self.color_genotype = np.random.choice(range(256), p=np.ones(256) / 256, size=4 * num_points)
        self.score = np.inf
        self.height = height
        self.width = width
        self.poly_n = 2*poly_count
    ...
    def get_params(self):
        return self.place_genotype.reshape((-1, self.poly_n)), self.color_genotype.reshape((-1, 4))
...
def make_offspring(individuals):
    ...
    for i in range(0, len(individuals), 2):
        ...
        off_1, off_2 = one_point_crossover(ind_1, ind_2)
...
def evaluate_individuals(individuals, ground_t,width,height, trasnparent ):
    for ind in individuals:
        ...
        if trasnparent:
            im = draw_poly_shaded(veroni_points,veroni_colors,width,height)
        else:
            painting = draw_poly(veroni_points,veroni_colors,width,height)
...
def draw_poly(genotype_coords, genotype_colors, img_width, img_height):
    canvas = numpy.ones((img_height,img_width,3),dtype=np.uint8)*255
    for i,poly in enumerate(genotype_coords):
        place = [[poly[j],poly[j+1]] for j in range(len(poly)-1)]
        pts = np.array([place],dtype=int)
        RED =(int(genotype_colors[i,0]),int(genotype_colors[i,1]),int(genotype_colors[i,2]))
        cv2.fillPoly(img=canvas,pts=[pts],color=RED)
    return canvas

def draw_poly_shaded(genotype_coords, genotype_colors, img_width, img_height):
    img = Image.new('RGB', (img_width, img_height),color = (255,255,255))
    drw = ImageDraw.Draw(img, 'RGBA')

    for i,poly in enumerate(genotype_coords):
        pts =  [(poly[j],poly[j+1]) for j in range(len(poly)-1)]
...
def main(display=False, interval = 100, verbose_interval = 1, transparent = False):
    ...
    individuals = [Individual(num_points,scaled_h,scaled_w,poly_count) for _ in range(num_individuals)]
    ...
    for i in range(itterations):
        ...
        if display and i % interval == 0:
            ...
            if not transparent:
                painting = draw_poly(veroni_points*(1/scale), veroni_colors, width, height)
            if transparent:
                painting = np.asarray(draw_poly_shaded(veroni_points * (1 / scale), veroni_colors, width, height))
        ...
        if i % verbose_interval == 0:
            print("Best score for itteration", i,  individuals[0].score, " :length of genotypes", len(individuals[0].place_genotype)/(2*poly_count ))
