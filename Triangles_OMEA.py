...
class Individual(object):
    ...
    def duplicate_genome(self):
        ...
        #self.place_genotype = np.append( copy.deepcopy(self.place_genotype), self.place_genotype)
        #self.color_genotype = np.append(copy.deepcopy(self.color_genotype), self.color_genotype )
...
def evaluate_individuals(individuals, ground_t,width,height, trasnparent):
    for ind in individuals:
        ...
        if trasnparent:
            im = draw_triangles_shaded(places,colors,width,height)
        else:
            painting = draw_triangles(places,colors,width,height)
...
def gene_optimal_mixing(individuals,ground_t,width, height,x,i = 0,asynch = False):
    ...
    for j in range(int(len(o_i.place_genotype)/6)):
...
def mutate_individuals(individuals,p=0.02):
    for ind in individuals:
...
def main(display=False, interval = 100, verbose_interval = 1, transparent = False):
    ...
    for i in range(itterations):
        ...
        if display and i % interval == 0:
            ...
            if not transparent:
                painting = draw_triangles(places*(1 / scale),colors, width, height,)
            if transparent:
                painting = np.asarray(draw_triangles_shaded(places*(1 / scale),colors, width, height))
